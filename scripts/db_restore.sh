...
usage() {
  ...
  echo "e.g.";
  echo "> db_restore.sh -d etudio_dvl -U etudio -f test/dbdumps/etudio_dvl.pgsql"
  exit 0;
}
...
LOCALDBUSER=${LOCALDBUSER:-"etudio"}
LOCALDBNAME=${LOCALDBNAME:-"etudio_dvl"}
DBDUMP_FILE=${DBDUMP_FILE:-"test/dbdumps/etudio_dvl.pgsql"}
...
# $> psql etudio_test -c "REASSIGN OWNED BY michael TO etudio;"
# Because the views created by the CIS extension are not owned by the etudio user
